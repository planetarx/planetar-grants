...
**(DO1) Real-time enforcement at tactical-decision speed.** The FCE enforces at the message chokepoint every fused element already crosses, benchmarked at p50 80–140 ns / p99 400–900 ns on its shared-memory path over a 1M-message run — orders of magnitude under any tactical-decision threshold. Because the policy check is synchronous and in-path, enforcement is real-time by construction across the demo's two modalities (AIS, SAR) and Protected B markings. The 1a measures FCE-on overhead against this baseline and reports it conservatively (bracketing measured runs, no post-tuning figures), so the "no added tactical latency" outcome is evidenced, not asserted.
